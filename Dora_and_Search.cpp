...
// double dist(double a,double b, double c, double d){return sqrt( (a-c)*(a-c) + (b-d)*(b-d) );}
...
void solve(){
    ...
    ll a[n];
    pll dp[6001];
    rep(i,0,6001) dp[i].first=-1,dp[i].second=1;
    map<ll,ll> mp;
    ...
    drep(i,n-1,0){
        ...
        if(mp[a[i]/3]>0 && dp[a[i]/3].first!=a[i]){
            dp[a[i]/3].first=a[i];
            dp[a[i]/3].second=dp[a[i]].second+1;
        }
        
    }
    ll ans=1;
    rep(i,0,mx+1) maxs(ans,dp[i].second);
    cout<<ans<<nl;
}
